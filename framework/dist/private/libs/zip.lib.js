...
Native.packer = {
	...
	fixedLitCodeTab : [new Uint32Array([
	...
	fixedDistCodeTab : [new Uint32Array([
	...
	deflate : function(bytes){
		...
		function ensureBuffer(requested) {
			var size = 512,
				current = buffer ? buffer.byteLength : 0;
		...
		function readBlock() {
			var eof = false,
				hdr = getBits(3),
				end, blockLen, check, b,
				litCodeTable, distCodeTable;
			...
			if (hdr == 1) {
				...
				litCodeTable = Native.packer.fixedLitCodeTab;
				distCodeTable = Native.packer.fixedDistCodeTab;
			} else if (hdr == 2) {
				// compressed block, dynamic codes
				var numLitCodes = getBits(5) + 257,
					numDistCodes = getBits(5) + 1,
					numCodeLenCodes = getBits(4) + 4,
					codeLenCodeLengths = new Uint8Array(Native.packer.codeLenCodeMap.length);

				for (var i = 0; i < numCodeLenCodes; ++i) {
					codeLenCodeLengths[Native.packer.codeLenCodeMap[i]] = getBits(3);
				}
			...
			while (true) {
				...
				code1 -= 257;
				code1 = Native.packer.lengthDecode[code1];

				var code2 = code1 >> 16;
				if (code2 > 0) code2 = getBits(code2);

				var len = (code1 & 0xffff) + code2;
				code1 = getCode(distCodeTable);
				code1 = Native.packer.distDecode[code1];
				code2 = code1 >> 16;
		...
		var source = new Uint8Array(buffer.buffer, 0, bufferLength);

		// shrink the buffer to the actual data size
		return {
			buffer : source.buffer.slice(0, bufferLength),
			data : source
		}
	}
};
...
function ZipFile(buffer) {
	...
	if (view.getInt32(0, true) != 0x04034b50) return;
	...
	while (true) {
		...
		if (view.getInt32(pos, true) == 0x06054b50) break;
		pos -= 4;
	}

	// read the central directory
	var directory = Object.create(null),
		entries = view.getInt16(pos + 8, true),
		pos = view.getInt32(pos + 16, true);

	while (entries--) {
		// check for directory entry signature
		if (view.getInt32(pos, true) != 0x02014b50) return;
		
		var compression_method = view.getInt16(pos + 10, true),
			compressed_len = view.getInt32(pos + 20, true),
			uncompressed_len = view.getInt32(pos + 24, true),
			filename_len = view.getInt16(pos + 28, true),
			extra_len = view.getInt16(pos + 30, true),
			comment_len = view.getInt16(pos + 32, true),
			local_header_offset = view.getInt32(pos + 42, true);
...
ZipFile.prototype = {
	read : function(filename){
		...
		switch (entry.compression_method){
			...
			case 8:
				// deflated
				return Native.packer.deflate(data).buffer;
		}
		return null;
	},

	list : function(callback){
		if (typeof callback == "function") callback(this.directory);
		return this.directory;
	}
};

File.unzip = function(filename, callback){
	var self = this;

	this.read(filename, function(buffer, size){

		var zip = new ZipFile(buffer);

		zip.extract = function(file, output){
			var data = this.read(file);
			var z = new File(output?output:file);
			z.open("w", function(){
