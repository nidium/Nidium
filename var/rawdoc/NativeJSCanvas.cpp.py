...
The parent of a canvas can be set by using methodls like 'Canvas.add'.
...
Only canvas instances that are descendens from the root canvas can be displayed.
...
The canvas remains in memory as long as it is a valid javascript instance. This way it can be re-attached to another canvas at a later stage.""",
	...
	[ExampleDoc("""canvas.removeFromParent(); """)],
	IS_Dynamic, IS_Public, IS_Fast,
	NO_Params,
	NO_Returns
)
...
If the canvas has no parent, null is returned.""",
	...
	[ExampleDoc("""var parent = canvas.getParent();""")],
	IS_Dynamic, IS_Public, IS_Fast,
	NO_Params,
...
The first child is the direct child with the lowest z-index.""", 
	SeesDocs("document.canvas|Canvas.add|Canvas.getParent|Canvas.getFirstChild|Canvas.getLastChild|Canvas.getNextSibling|Canvas.getPrevSibling" ),
	[ExampleDoc("""var child = canvas.getFirstChild();""")],
	IS_Dynamic, IS_Public, IS_Fast,
	NO_Params,
...
The last child is the direct child with the highest z-index.""", 
	SeesDocs("document.canvas|Canvas.add|Canvas.getParent|Canvas.getFirstChild|Canvas.getLastChild|Canvas.getNextSibling|Canvas.getPrevSibling" ),
	[ExampleDoc("""var child = canvas.getLastChild();""")],
	IS_Dynamic, IS_Public, IS_Fast,
	NO_Params,
...
FunctionDoc( "Canvas.getNextSibling", """Find the next sibling of a canvas object.""",
	SeesDocs("document.canvas|Canvas.add|Canvas.getParent|Canvas.getFirstChild|Canvas.getLastChild|Canvas.getNextSibling|Canvas.getPrevSibling" ),
	[ExampleDoc("""document.canvas.add(canvas1);
document.canvas.add(canvas2);
...
FunctionDoc( "Canvas.setCoordinates", """Set a canvas object to a certain position.
...
The behavior depends on the 'Canvas.position' type.""",
	...
	[ExampleDoc("""canvas.position = 'absolute';
...
If a canvas is already attached to a parent, it's removed from the old one.""",
...
""",
	...
	[ExampleDoc("""var context = canvas.getContext("2d");
...
FieldDoc( "Canvas.left", """Get or set the top-left position of the canvas.
...
FieldDoc( "Canvas.right", """Get or set the top-right corner position of the canvas.
...
This property defines whether children are clipped inside the canvas boundaries or not.""",
	SeesDocs( "Canvas.visible|Canvas.opacity|Canvas.overflow|Canvas.coating" ),
	[ExampleDoc("""var canvas = new Canvas(100, 100);
document.canvas.add(canvas);
