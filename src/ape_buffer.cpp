...
buffer *buffer_new(size_t size)
{
    ...
    if ((b->size = size) > 0) {
        b->data = (unsigned char *)malloc(sizeof(char) * size);
    } else {
        b->size = 0;
    }
...
void buffer_prepare(buffer *b, size_t size)
{
    if (b->size == 0) {
        b->size = size;
        b->used = 0;
        b->data = (unsigned char *)malloc(sizeof(char) * b->size);
    } else if (b->used + size > b->size) {
        b->size += size;
        b->data = (unsigned char *)realloc(b->data, sizeof(char) * b->size);
    }
}
...
buffer *buffer_to_buffer_utf8(buffer *b)
{
    ...
    if (newb->size > newb->used+1) {
        ...
        newb->data = (unsigned char *)realloc(newb->data, newb->size);
    }
