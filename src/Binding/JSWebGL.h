...
struct NGLContextAttributes
{
    ...
    void set(bool alpha,
             ...
             bool preferLowPowerToHightPerformance)
    {
        m_Alpha                            = alpha;
        m_Depth                            = depth;
        m_Stencil                          = stencil;
...
{
public:
    CanvasWebGLContext(JSContext *cx,
                       NGLContextAttributes *attributes,
                       int width,
                       int height);
    ~CanvasWebGLContext();
...
public:
    JSWebGLActiveInfo();
    ~JSWebGLActiveInfo();
    static JS::HandleObject
    Create(JSContext *cx, GLint size, GLenum type, const char *name);
    static void RegisterObject(JSContext *cx);
};
